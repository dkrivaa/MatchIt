...
def step2():
    ...
    if file is not None:
        ...
        invalid_values_condition = df1.apply(lambda col: ~col.apply(lambda x: isinstance(x, int) or x == 999))
        invalid_values = df1[
            df1.iloc[:, 1:].apply(lambda row: any(row != 999), axis=1) & invalid_values_condition.any(axis=1)]
